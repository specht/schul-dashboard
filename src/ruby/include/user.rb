    ...
    def print_tresor_countdown_panel()
        ...
        if Time.now.strftime('%Y-%m-%dT%H:%M:%S') <= deadline
            return StringIO.open do |io|
                ...
                io.puts "<hr />"
                io.puts "<p>Die Möglichkeit für Eintragungen der Noten für das Arbeits- und Sozialverhalten endet am Montag um 20:00 Uhr. Bitte tragen Sie bis dahin fehlende Noten ein, damit die Klassenleitungen bis zu den Zeugniskonferenzen die Listen drucken können.</p>"
                io.puts "<div id='tresor_countdown_here' style='display: none;' data-deadline='#{Time.parse(deadline).to_i}'>"
                io.puts "</div>"
                io.puts "</div>"
                io.puts "</div>"
