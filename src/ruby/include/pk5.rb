    ...
    def print_pending_pk5_invitations_incoming(user_email)
        ...
        StringIO.open do |io|
            ...
            invitations.values.each do |emails|
                ...
                io.puts "<p>"
                io.puts "<button class='btn btn-success bu-accept-invitation' data-email='#{emails.first}'><i class='bi bi-check-lg'></i>&nbsp;&nbsp;Einladung annehmen</button>"
                io.puts "<button class='btn btn-danger bu-reject-invitation' data-email='#{emails.first}'><i class='bi bi-x-lg'></i>&nbsp;&nbsp;Einladung ablehnen</button>"
                io.puts "</p>"
            end
            io.puts "<hr>"
            io.string
    ...
    def pending_pk5_invitations_outgoing(user_email)
        ...
        StringIO.open do |io|
            ...
            pending_invitations.each do |row|
                ...
                io.puts "<p>"
                io.puts "<button class='btn btn-danger bu-delete-invitation' data-email='#{row['u.email']}'><i class='bi bi-x-lg'></i>&nbsp;&nbsp;Einladung zurücknehmen</button>"
                io.puts "</p>"
            end
            io.puts "<hr>"
            io.string
