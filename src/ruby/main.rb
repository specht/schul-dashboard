...
require '/data/unterstufenparty/config.rb'
...
require './include/unterstufenparty.rb'
...
class Main < Sinatra::Base
    ...
    configure do
        ...
        if ENV['DASHBOARD_SERVICE'] == 'ruby' && ['pry.rb', 'puma', 'rackup'].include?(File.basename($0))
            setup.setup(self)
        ...
        if ['puma', 'rackup'].include?(File.basename($0))
            ...
            nav_items.each do |x|
                if x == :admin
                    ...
                    if admin_logged_in?
                        ...
                        io.puts "<a class='dropdown-item nav-icon' href='/inactive_sus'><div class='icon'><i class='fa fa-question-circle-o'></i></div><span class='label'>Inaktive SuS</span></a>"
