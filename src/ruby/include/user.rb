class Main < Sinatra::Base
    ...
    def user_icon(email, c = nil)
        "<div style='background-image: url(#{NEXTCLOUD_URL}/index.php/avatar/#{@@user_info[email][:nc_login]}/64), url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mO88h8AAq0B1REmZuEAAAAASUVORK5CYII=);' class='#{c}'></div>"
    end
    ...
    def print_tresor_countdown_panel()
        ...
        if Time.now.strftime('%Y-%m-%dT%H:%M:%S') <= deadline && (DateTime.parse(deadline) - DateTime.now).to_f < 7.0
            parts << StringIO.open do |io|
                ...
                io.puts "<p>Die Noteneingabe im Datentresor schließt am <strong>#{WEEKDAYS_LONG[d.wday]} um #{d.strftime('%H:%M')} Uhr</strong>.</p>"
        ...
        if need_sozialverhalten()
            ...
            if Time.now.strftime('%Y-%m-%dT%H:%M:%S') <= deadline && (DateTime.parse(deadline) - DateTime.now).to_f < 7.0
                parts << StringIO.open do |io|
                    ...
                    io.puts "<hr />"
                    d = DateTime.parse(deadline)
                    io.puts "<p>Die Möglichkeit für Eintragungen der Noten für das Arbeits- und Sozialverhalten endet am <strong>#{WEEKDAYS_LONG[d.wday]} um #{d.strftime('%H:%M')} Uhr</strong>. Bitte tragen Sie bis dahin fehlende Noten ein, damit die Klassenleitungen rechtzeitig vor der Zeugnisausgabe die Sozialzeugnisse drucken können.</p>"
                    io.puts "<div id='tresor_countdown_here' style='display: none;' data-deadline='#{Time.parse(deadline).to_i}'>"
                    io.puts "</div>"
                    io.puts "</div>"
                    io.puts "</div>"
