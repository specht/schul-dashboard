...
class Main < Sinatra::Base
    ...
    def get_lab9_projekte
        projekte = {}
        neo4j_query(<<~END_OF_QUERY).each do |row|
            MATCH (p:Lab9Projekt)-[:BELONGS_TO]->(u:User)
            RETURN p, u.email;
        END_OF_QUERY
            p = row['p']
    ...
    post '/api/get_lab9_projekte' do
        require_user!
        result = {:projekte => get_projekte()}
        respond(result)
    end    

    def get_my_lab9_projekt(email)
        require_user!
        result = nil
        neo4j_query(<<~END_OF_QUERY, {:email => email}).each do |row|
            MATCH (p:Lab9Projekt)-[:BELONGS_TO]->(u:User {email: $email})
            WITH p
    ...
    def my_lab9_projekt_history(email)
        StringIO.open do |io|
            entries = neo4j_query(<<~END_OF_QUERY, {:email => email}).to_a
                MATCH (eu:User)<-[:BY]-(pc:Lab9ProjektChange)-[:TO]->(p:Lab9Projekt)-[:BELONGS_TO]->(u:User {email: $email})
                RETURN pc, eu.email
                ORDER BY pc.ts;
            END_OF_QUERY
            if entries.empty?
                ...
                entries.each do |entry|
                    ...
                    if pc[:type] == 'update_value'
                        ...
                        value = pc[:value]
                        if value.nil?
                            io.puts "<div class='history_entry'>#{LAB9_KEY_LABELS[key]} gelöscht durch #{@@user_info[entry['eu.email']][:display_name_official_dativ]}</div>"
                        else
                            io.puts "<div class='history_entry'>#{LAB9_KEY_LABELS[key]} geändert auf <strong>»#{value}«</strong> durch #{@@user_info[entry['eu.email']][:display_name_official_dativ]}</div>"
                        end
    ...
    post '/api/my_lab9_projekt_history' do
        require_user!
        ...
        respond(:html => my_lab9_projekt_history(sus_email))
    end

    post '/api/update_lab9_projekt' do
        require_user!
        data = parse_request_data(
            :optional_keys => [
                :sus_email,
        ...
        assert(email_is_eligible_for_lab9?(@@user_info, sus_email))
        ...
        transaction do
            p = neo4j_query_expect_one(<<~END_OF_QUERY, {:sus_email => sus_email})['p']
                ...
                MERGE (p:Lab9Projekt)-[:BELONGS_TO]->(u)
                RETURN p;
            END_OF_QUERY
            LAB9_KEYS.each do |key|
                if data.include?(key)
                    value = data[key]
                    debug "#{key} => #{value}"
                    if (value || '') != (p[key] || '')
                        neo4j_query_expect_one(<<~END_OF_QUERY, {:sus_email => sus_email, key => value})
                            MATCH (p:Lab9Projekt)-[:BELONGS_TO]->(u:User {email: $sus_email})
                        ...
                        neo4j_query_expect_one(<<~END_OF_QUERY, {:sus_email => sus_email, :key => key, :value => value, :ts => ts, :editor_email => @session_user[:email]})
                            ...
                            MATCH (p:Lab9Projekt)-[:BELONGS_TO]->(:User {email: $sus_email})
                            CREATE (eu)<-[:BY]-(c:Lab9ProjektChange)-[:TO]->(p)
        ...
        respond(:yay => 'sure', :result => get_my_lab9_projekt(sus_email))
    end

    def lab9_overview_rows
        assert(teacher_logged_in? || email_is_eligible_for_lab9?(@@user_info, @session_user[:email]))
        ...
        @@user_info.keys.each do |email|
            next unless email_is_eligible_for_lab9?(@@user_info, email)
    ...
    post '/api/lab9_overview' do
        assert(teacher_logged_in? || email_is_eligible_for_lab9?(@@user_info, @session_user[:email]))

        rows = lab9_overview_rows()
        rows.sort! do |a, b|
            a_nr = a[:projekt][:nr] || ''
            b_nr = b[:projekt][:nr] || ''
    ...
    post '/api/send_invitation_for_lab9_projekt' do
        data = parse_request_data(:required_keys => [:sus_email, :name])
        sus_email = data[:sus_email]
        if user_with_role_logged_in?(:can_manage_lab9)
    ...
    post '/api/delete_invitation_for_lab9_projekt' do
    ...
    def print_pending_lab9_projekt_invitations_incoming(user_email)
        pending_invitations = neo4j_query(<<~END_OF_QUERY, {:email => user_email}).to_a
            MATCH (ou:User)<-[:BELONGS_TO]-(p:Lab9Projekt)-[r:INVITATION_FOR]->(u:User {email: $email})
        ...
        StringIO.open do |io|
            ...
            invitations.values.each do |emails|
                io.puts "<p>Du hast eine Einladung von <strong>#{join_with_sep(emails.map { |x| @@user_info[x][:display_name]}, ', ', ' und ')}</strong> für ein gemeinsames Lab 9-Projekt erhalten.</p>"
                io.puts "<p>"
    ...
    def pending_lab9_projekt_invitations_outgoing(user_email)
        pending_invitations = neo4j_query(<<~END_OF_QUERY, {:email => user_email}).to_a
            MATCH (ou:User {email: $email})<-[:BELONGS_TO]-(p:Lab9Projekt)-[r:INVITATION_FOR]->(u:User)
            RETURN u.email;
        END_OF_QUERY
        return '' if pending_invitations.empty?
        StringIO.open do |io|
            io.puts "<hr>"
            pending_invitations.each do |row|
                io.puts "<p>Du hast <strong>#{@@user_info[row['u.email']][:display_name]}</strong> für dein Lab 9-Projekt eingeladen.</p>"
                io.puts "<p>"
    ...
    post '/api/pending_lab9_projekt_invitations_outgoing' do
        ...
        respond(:html => pending_lab9_projekt_invitations_outgoing(sus_email))
    end

    post '/api/accept_lab9_projekt_invitation' do
        data = parse_request_data(:required_keys => [:sus_email, :other_email])
        sus_email = data[:sus_email]
        if user_with_role_logged_in?(:can_manage_lab9)
        elsif user_with_role_logged_in?(:schueler)
        ...
        transaction do
            neo4j_query(<<~END_OF_QUERY, {:sus_email => sus_email, :other_email => other_email})
                MATCH (ou:User {email: $other_email})<-[:BELONGS_TO]-(p:Lab9Projekt)-[r:INVITATION_FOR]->(u:User {email: $sus_email})
                DELETE r
            ...
            neo4j_query_expect_one(<<~END_OF_QUERY, {:sus_email => sus_email, :other_email => other_email, :ts => ts, :editor_email => @session_user[:email]})
                MATCH (eu:User {email: $editor_email})
                MATCH (p:Lab9Projekt)-[:BELONGS_TO]->(:User {email: $other_email})
                CREATE (eu)<-[:BY]-(c: Lab9ProjektChange)-[:TO]->(p)
    ...
    post '/api/reject_lab9_projekt_invitation' do
        data = parse_request_data(:required_keys => [:sus_email, :other_email])
        sus_email = data[:sus_email]
        if user_with_role_logged_in?(:can_manage_lab9)
        elsif user_with_role_logged_in?(:schueler)
